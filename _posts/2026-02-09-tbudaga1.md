...
This data visualization shows how high loading and low loading cells on PC1 relate to each other in a spatial distribution across the tissue. To find where they were spatially located, I found the type of gene that dominated in that location (high loading vs low loading). 
...
tend to be in the middle of the tissue while the high loading genes are primarily found on the outer edges of the tissue. This suggests that the  high loading and low loading gene sets tend to show opposing expression patterns across the tissue.
...
The histograms show that the high loading genes and low loading genes are almost opposite. As one set is strongly expressed, the other set is usually weaker. T Ultimately, both the spatial plots and the histograms show that these two gene sets have contrasting gene expression patterns in the tissue.  
